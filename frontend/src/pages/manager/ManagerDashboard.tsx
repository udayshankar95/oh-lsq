...
const STATE_COLORS: Record<string, string> = {
  NEW: 'bg-gray-300',
  ATTEMPTING: 'bg-[#E8762C]',
  CONNECTED: 'bg-blue-400',
  SCHEDULED: 'bg-green-500',
  CALLBACK_SCHEDULED: 'bg-gray-400',
...
const STATE_TEXT: Record<string, string> = {
  NEW: 'text-gray-500',
  ATTEMPTING: 'text-[#E8762C]',
  CONNECTED: 'text-blue-600',
  SCHEDULED: 'text-green-600',
  CALLBACK_SCHEDULED: 'text-gray-600',
...
function StatCard({ label, value, sub, alert }: { label: string; value: number | string; sub?: string; alert?: boolean }) {
  return (
    <div className={`border p-4 bg-white ${alert ? 'border-red-300' : 'border-gray-200'}`}>
      <p className={`text-xs font-semibold uppercase tracking-wide mb-1 ${alert ? 'text-red-500' : 'text-gray-400'}`}>{label}</p>
    ...
    <div className="p-6 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          ...
          className="flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-700 px-3 py-1.5 border border-gray-200 hover:border-gray-300 bg-white transition-colors"
        >
      ...
      {(metrics.queue.overdue_callbacks > 0 || metrics.queue.waiting_over_24h > 0) && (
        <div className="mb-5 flex flex-wrap gap-3">
          {metrics.queue.overdue_callbacks > 0 && (
            <div className="flex items-center gap-2 bg-red-50 border border-red-200 text-red-700 text-sm font-medium px-3 py-2">
          ...
          {metrics.queue.waiting_over_24h > 0 && (
            <div className="flex items-center gap-2 bg-amber-50 border border-amber-200 text-amber-700 text-sm font-medium px-3 py-2">
      ...
      <div className="bg-white border border-gray-200 p-5">
        <h2 className="text-sm font-semibold text-gray-700 uppercase tracking-wide mb-4">Lead Pipeline</h2>
        <div className="space-y-2.5">
          {metrics.state_breakdown.map(({ state, count }) => {
            const pct = metrics.leads.total > 0 ? (count / metrics.leads.total) * 100 : 0;
            return (
              <div key={state} className="flex items-center gap-3">
                <span className={`text-xs font-medium w-36 ${STATE_TEXT[state] || 'text-gray-500'}`}>
                  {state.replace(/_/g, ' ')}
                </span>
                <div className="flex-1 bg-gray-100 h-1.5 overflow-hidden">
                  <div
                    className={`h-1.5 transition-all duration-500 ${STATE_COLORS[state] || 'bg-gray-300'}`}
