...
export default function AgentQueue() {
  ...
  const filteredTasks = tasks.filter(t => {
    ...
    if (queueFilter === 'overdue') return isOverdue(t);
    return t.type === queueFilter;
  });
    ...
    <div className="h-full flex flex-col bg-gray-50">
      ...
      <div className="w-80 flex-shrink-0 flex flex-col border-r border-gray-200 bg-white">
        <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between flex-shrink-0">
          <div>
            <h1 className="text-sm font-bold text-gray-900">My Queue</h1>
            <p className="text-xs text-gray-400 mt-0.5">{tasks.length} task{tasks.length !== 1 ? 's' : ''}</p>
          </div>
          <button onClick={() => fetchQueue()} className="p-1.5 text-gray-400 hover:text-gray-600 transition-colors">
          ...
          <div className="px-3 py-2 border-b border-gray-100 flex flex-wrap gap-1.5 flex-shrink-0">
            {[
              ...
              const count = chip.id === 'all'
                ...
                : tasks.filter(t => t.type === chip.id).length;
              ...
              return (
                ...
                >
                  {chip.label} {count > 0 && chip.id !== 'all' ? `(${count})` : ''}
                </button>
              );
            })}
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1.5">
          {loadingQueue ? (
            <div className="flex justify-center py-10">
              <svg className="animate-spin w-5 h-5 text-[#E8762C]" fill="none" viewBox="0 0 24 24">
                <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/>
                <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/>
              </svg>
            </div>
          ) : queueError ? (
            <p className="text-xs text-red-500 px-2 py-2">{queueError}</p>
          ) : tasks.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-xs text-gray-400 mt-1">Tasks appear here automatically</p>
            </div>
          ) : (
            filteredTasks.map((task, index) => (
              <button
                ...
                className={`w-full text-left rounded-xl border transition-all p-3 ${
                  selectedId === task.id
                    ? 'border-[#E8762C] bg-orange-50 ring-1 ring-orange-200'
                    : isOverdue(task)
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  <div className="w-7 h-7 rounded-full bg-orange-50 flex items-center justify-center flex-shrink-0">
                    <span className="text-xs font-semibold text-[#E8762C]">{task.patient_name?.charAt(0)}</span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-gray-900 truncate">{task.patient_name}</p>
